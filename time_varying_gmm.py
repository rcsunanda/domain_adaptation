...
class GaussianMixtureModel(st.rv_continuous):
    ...
    class Component:
        ...
        def setParams(self, weight, mean, std):
            ...
            self.RV = st.norm(loc=mean,
                              scale=std)  # for now create new Gaussian, but this should be changed to proper setters
    ...
    def _rvs(self):
        ...
        print("GaussianMixtureModel::_rvs; numBatches=%d; sampleCount=%d; batchSize=%d" % (
        numBatches, sampleCount, batchSize))

        for i in range(numBatches):
            # Select a component according to the distribution given by weights
            componentNum = np.random.choice(np.arange(0, len(self.componentList)),
                                            p=[comp.weight for comp in self.componentList])
        ...
        if currSampleCount < sampleCount:
            ...
            print("GaussianMixtureModel::_rvs; Some more to be sampled; currSampleCount=%d; remainingSampleCount=%d" % (
            currSampleCount, remainingSampleCount))
